...
var (
	...
	newrelicLicense = flag.String("newrelic-license", "", "Newrelic license")
...
func init() {
	...
	flag.Usage = func() {
		fmt.Fprintln(os.Stderr, "Usage: example [-cert=<cert>] [-key=<key>] [-config=<config>] [-listen=<listen>] [-newrelic-license=<newrelic-license>]")
		flag.PrintDefaults()
	}
